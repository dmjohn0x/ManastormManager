...
ManastormManagerDB = ManastormManagerDB or {
    delay = 0.4,  -- Delay between opening caches (seconds)
...
local function CountdownPrint(remaining, isAutoOpen)
    -- Only show countdown if not auto-opening or if verbose logging is enabled
    if not isAutoOpen or ManastormManagerDB.verbose then
        print("|cff00ff00[Millhouse Manastorm]|r Behold my magical prowess! |cffff9933" .. remaining .. "|r Manastorm Cache" .. (remaining > 1 and "s" or "") .. " remaining to open!")
    end
end
...
local function FindManastormCaches()
        ...
        if numSlots then
            for slot = 1, numSlots do
                ...
                if itemLink then
                    ...
                    if itemName and string.find(itemName, "Manastorm") and string.find(itemName, "Cache") then
                        local texture, itemCount, locked, quality, readable = GetContainerItemInfo(bag, slot)
                        if not locked and itemCount and itemCount > 0 then
                            -- Check if this is a Bonus Manastorm Cache
                            if string.find(itemName, "Bonus") then
                                bonusCaches = bonusCaches + itemCount
                            else
                                -- Regular Manastorm Cache that can be opened
    ...
    -- If we found bonus caches, inform the player
    if bonusCaches > 0 then
...
local function OpenCache(bag, slot, isAutoOpen)
    ...
    if remaining > 0 then
        CountdownPrint(remaining, isAutoOpen)
    end
    -- Completion message is handled in ProcessQueue
end
...
local function OpenManastormCaches(isAutoOpen)
    if isOpening then
        if not isAutoOpen then
            Print("My magical powers are already at work opening caches! Even I cannot do two things at once!")
        end
        return
    end
    ...
    if not isAutoOpen or ManastormManagerDB.verbose then
        Print("Excellent! I have found " .. totalCaches .. " Manastorm Cache" .. (totalCaches > 1 and "s" or "") .. " to demonstrate my power upon!")
        CountdownPrint(totalCaches, isAutoOpen)  -- Show initial countdown
    end
            ...
            for slot = 1, numSlots do
                ...
                if texture and not locked then
                    ...
                    if itemLink then
                        ...
                        -- Auto-sell items are sold regardless of quality, equipment items only if quality is right
                        ...
                        elseif quality and ShouldSellQuality(quality) and itemEquipLoc and itemEquipLoc ~= "" and vendorPrice and vendorPrice > 0 and not isProtected then
                            shouldSell = true
...
local function VendorEquipment()
    ...
    if table.getn(items) == 0 then
        Print("Your bags contain no equipment worthy of my transmutation magic!")
        return
    end
    ...
    Print("Excellent! I have found " .. totalVendorItems .. " pieces of " .. qualityText .. " equipment to transmute into gold through my superior magic!")
...
local function CountVendorItems()
    ...
    if count > 0 then
        Print("My superior senses detect " .. count .. " piece" .. (count > 1 and "s" or "") .. " of equipment ready for my transmutation magic!")
    else
        Print("Your bags contain no equipment worthy of my incredible powers.")
    end
end
...
local function CreateOptionsGUI()
    ...
    -- Show dock checkbox (moved up since we're removing keepRares)
    local showDockCheck = CreateFrame("CheckButton", nil, optionsFrame, "UICheckButtonTemplate")
    showDockCheck:SetPoint("TOPLEFT", 30, -120)
    ...
    themeDropdown:SetPoint("TOPLEFT", 10, -190)
    ...
    delaySlider:SetPoint("TOPLEFT", 30, -250)
    ...
    vendorDelaySlider:SetPoint("TOPLEFT", 30, -320)
    ...
    sellAllLabel:SetPoint("TOPRIGHT", -30, -180)
...
local function CreateProtectedItemsGUI()
    ...
    addLabel:SetPoint("TOPLEFT", 20, -90)
...
-- Check if bags are closed and stop cache opening if needed
local function CheckBagsClosed()
    if isOpening and not IsBagOpen() then
        DebugPrint("Bags closed, stopping cache opening process.")
        isOpening = false
        openQueue = {}
        currentlyOpening = 0
        totalCaches = 0
